...
public class InventoryManager {
    ...
    public boolean removeStock(String item, int qty) {
        ...
        stock.compute(item, (key,current_qty)-> {
            if (current_qty != null && current_qty >= qty) {
                // we set the outside flag thing to true - indicating we removed stock bc we had sufficeint amount
                removed[0] = true;
                return current_qty - qty;
            }
            // if theres not enough stock - then dont remove any more
            return current_qty;
        });

        return removed[0]; // we return the boolean so we know if the removal was successful or not
    }
    ...
    public int getStock(String item) {
        return stock.getOrDefault(item, 0); // this is atomic. try-catch is not.
    }
